...
def bump_alignment_loss(bump_activity, target_angle, num_neurons):
    """
    Direct alignment between bump and target angle using cosine encoding.
    Using cosine instead of von Mises naturally penalizes multiple bumps.
    
    Args:
        bump_activity: (batch, seq_len, num_neurons) - the raw bump activity
        target_angle: (batch, seq_len) - target angles in radians
        num_neurons: number of neurons in the ring
    """
    batch_size, seq_len, _ = bump_activity.shape
    
    # Normalize bump to probability distribution
    bump_norm = bump_activity / (bump_activity.sum(dim=-1, keepdim=True) + 1e-8)
    
    # Create neuron positions around the ring
    neuron_angles = torch.linspace(0, 2*np.pi, num_neurons, device=bump_activity.device)
    neuron_angles = neuron_angles.unsqueeze(0).unsqueeze(0)  # (1, 1, num_neurons)
    target_angle_expanded = target_angle.unsqueeze(-1)  # (batch, seq_len, 1)
    
    # Use COSINE as the target pattern (not von Mises!)
    # This has negative values which penalize misaligned bumps
    target_cosine = torch.cos(neuron_angles - target_angle_expanded)
    
    # Compute alignment via dot product
    # With cosine, perfect alignment gives ~0.5, multiple bumps give negative values
    alignment = torch.sum(bump_norm * target_cosine, dim=-1)  # (batch, seq_len)
    
    # Scale and shift so that:
    # - Perfect single bump alignment: alignment ≈ 0.5 → loss ≈ 0.5
    # - Random/flat distribution: alignment ≈ 0 → loss ≈ 1.0
    # - Multiple bumps: alignment < 0 → loss > 1.0 (penalty!)
    loss = 1 - 2 * alignment.mean()  # Scale by 2 to make the range more meaningful
...
class GeneralizedRingAttractorNoGain(nn.Module):
    ...
    def __init__(self, num_neurons, action_dim=2, tau=10.0, dt=1.0, activation='tanh',
        ...
        self.weight_scale = num_neurons  # np.sqrt(num_neurons)
        # Fixed parameters
        # For Relu
    ...
    def forward(self, action_signal, r_init=None):
        ...
        r_history = []
        bump_history = []
        self.h = r.clone()  # For leaky integration
        ...
        for t in range(seq_len):
            ...
            # Recurrent dynamics
            recurrent_input = (W_eff @ r.unsqueeze(2)).squeeze(2) + self.recurrent_bias
            ...
            # Diffusion update
            # self.h = 0.5 * self.h + 0.5 * self.h_sm
            r = non_linear(self.h, self.activation_name)
...
def run_training_and_evaluation(action_dim=2):
    ...
    for step in range(training_steps):
        ...
        # bump_align_loss = bump_alignment_loss(bump_activity, target_angle, num_neurons)

        total_loss = loss + 0.2 * bump_loss # + 1.0 * bump_align_loss
        ...
        if step % 100 == 0:
            print(f"Step {step}/{training_steps}, Total Loss: {total_loss.item():.4f}, "
                  ...
                  f"Recurrent bias: {ring_rnn.recurrent_bias.item():.4f}"
                #   f"J1: {ring_rnn.J1.item():.2f}")
                  )
